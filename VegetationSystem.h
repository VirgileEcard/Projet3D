...
struct VoxelModel
{
    struct Part
    {
        int dx, dy, dz; // Position relative à la racine
        float r, g, b;  // Couleur
    };
...
enum PlantType
{
    ...
    TREE_PINE,   // Pin (Sec/Montagne)
    TREE_WILLOW, // Saule (Humide)
    BUSH_BERRY,  // Buisson
    GRASS_TUFT,  // Touffe d'herbe
    CACTUS       // Cactus
};
...
void initPlantModels()
{
    ...
    // Tronc
    for (int y = 1; y <= 4; y++)
        oak.parts.push_back({0, y, 0, 0.4f, 0.2f, 0.1f});
        ...
        for (int y = 3; y <= 5; y++)
        {
            for (int z = -2; z <= 2; z++)
            {
                if (abs(x) + abs(y - 4) + abs(z) <= 3) // Forme un peu arrondie
                    oak.parts.push_back({x, y, z, 0.1f, 0.6f, 0.1f});
            }
        }
    }
    ...
    VoxelModel pine;
    for (int y = 1; y <= 6; y++)
        pine.parts.push_back({0, y, 0, 0.3f, 0.15f, 0.05f});
    // Feuilles (Pyramide)
    for (int y = 3; y <= 7; y++)
    {
        int radius = 2 - (y - 3) / 2;
        for (int x = -radius; x <= radius; x++)
        {
            for (int z = -radius; z <= radius; z++)
            {
                ...
                pine.parts.push_back({x, y, z, 0.05f, 0.3f, 0.1f});
            }
        }
    }
    ...
    VoxelModel willow;
    for (int y = 1; y <= 3; y++)
        willow.parts.push_back({0, y, 0, 0.35f, 0.25f, 0.15f});
    // Canopée large
    for (int x = -2; x <= 2; x++)
    {
        for (int z = -2; z <= 2; z++)
        {
            willow.parts.push_back({x, 4, z, 0.2f, 0.5f, 0.1f});
            // Branches qui tombent
            if (abs(x) == 2 || abs(z) == 2)
            {
                willow.parts.push_back({x, 3, z, 0.2f, 0.55f, 0.1f});
                if ((x + z) % 2 != 0)
                    willow.parts.push_back({x, 2, z, 0.2f, 0.6f, 0.1f});
            }
        }
    }
